...
class Point < GeometryValue
  ...
  def intersectPoint p
    if real_close_point(@x,@y,p.x,p.y)
      ...
      intersectNoPoints NoPoints.new
    end
  end
  def intersectLine line
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    (inbetween(@x,seg.x1,seg.x2) and inbetween(@y,seg.y1,seg.y2)) ? self : intersectNoPoints(NoPoints.new)
  end
end
...
class Line < GeometryValue
  ...
  def intersectLine line
    if real_close(@m,line.m)
      real_close(@b,line.b) ? self : intersectNoPoints(NoPoints.new)
    else
      x = (line.b - @b) / (@m - line.m)
      y = @m * x + @b
      Point.new(x,y)
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(seg.x1,seg.x2)
      ...
      if real_close(segS.y2,segF.y1)
        Point.new(segS.x2,segS.y2)
      elsif segS.y2 < segF.y1
        intersectNoPoints NoPoints.new
      elsif segS.y2 > segF.y2
        segF
      else
        LineSegment.new(segF.x1,segF.y1,segS.x2,segS.y2)
      end
    else
      if seg.x1 < self.x1
        segS = seg
      ...
      if real_close(segS.x2,segF.x1)
        Point.new(segS.x2,segS.y2)
      elsif segS.x2 < segF.x1
        intersectNoPoints NoPoints.new
      elsif segS.x2 > segF.x2
        segF
      else
        LineSegment.new(segF.x1,segF.y1,segS.x2,segS.y2)
      end
